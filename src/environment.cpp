...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer, ProcessPointClouds<pcl::PointXYZI>* point_ProcessorI, const pcl::PointCloud<pcl::PointXYZI>::Ptr& inputCloud)
{
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr filtered_inputCloud;
    filtered_inputCloud = point_ProcessorI->FilterCloud(inputCloud, 0.2, Eigen::Vector4f (-11, -6, -3, 1), Eigen::Vector4f (36, 7, 2, 1));
    std::pair<pcl::PointCloud<pcl::PointXYZI>::Ptr,pcl::PointCloud<pcl::PointXYZI>::Ptr> segmentCloud = point_ProcessorI->SegmentPlane(filtered_inputCloud, 25, 0.3);

    //renderPointCloud(viewer,inputCloud, "cloud");
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters = point_ProcessorI->Clustering(segmentCloud.first, 0.53, 10, 2000);
    //renderPointCloud(viewer, segmentCloud.first,"obstCloud",Color(1,0,0));
    renderPointCloud(viewer, segmentCloud.second,"planeCloud", Color(1,1,0));

    int clusterId = 0;
    std::vector<Color> colors = {Color(0,1,0), Color(0,1,1), Color(1,0,1)};
...
int main (int argc, char** argv)
{
    ...
    while (!viewer->wasStopped ())
    {
        ...
        inputCloudI = point_ProcessorI->loadPcd((*streamIterator).string());
        cityBlock(viewer, point_ProcessorI, inputCloudI);
        streamIterator++;
